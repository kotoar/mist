...
import { CaseJudgeSchema } from "@lib/case/schema";
import { repairText } from "@lib/shared/services/ai-utils";
import { z } from "zod";

const JudgeWithHintSchema = z.object({
    correct: z.boolean().describe("Whether the answer is correct"),
    score: z.number().min(0).max(100).describe("Score for the answer, from 0 to 100"),
...
export async function judge(props: {
    ...
    story: string,
    includeHint?: boolean,
}): Promise<{ correct: boolean; score: number; hint?: string }> {
    const { input, question, referenceAnswer, puzzle, story, includeHint } = props;
    const prompt = [
    ...
    - 不可自行脑补未出现的更具体手段；若玩家未提及参考中的温和限定，不视为缺失核心。`,
        includeHint ? `### 提示 hint：
    - 如果答案不正确，给出简短提示，指出玩家回答中缺失或错误的关键点。
    - 如果答案正确，则不需要提供提示。` : undefined,
        `### 输出格式（仅此 JSON，不添加额外字符）：
    {
      "correct": true | false,
      "score": number${includeHint ? ',\n      "hint": "optional string"' : ''}
    }`
    ].filter(Boolean).join("\n\n");

    const schema = includeHint ? JudgeWithHintSchema : CaseJudgeSchema;
    ...
    return response.object as { correct: boolean; score: number; hint?: string };
}
